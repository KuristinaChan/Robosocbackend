...
let handleDelete = (id, name)=>{
    ...
    deleteDivText[0].textContent = `Are you sure you want to delete project ${name} ?` 
    idOfprojectToBeDeleted = id
}

let deleteproject = ()=>{
    deleteDiv[0].style.display = "none"
    fetch(`/api/projects?id=${idOfprojectToBeDeleted}`,{
        "method" : "DELETE"
    })
    .then((res)=>{
        if(!res.ok){
        ...
        .then((res) => {
            ...
            projectsData.forEach((data) => {
                ...
                <div class="single-project">
                    ...
                    <div class="options">
                    <!--Edit--icon-->                        
                        <div onclick="handleEdit('${data._id}', '${data.name}', '${data.description.replace(/\n/g, ' ')}','${data.image}')" id="edit">
...
let handleEdit = (id,name,description,image)=>{
    ...
    projectToBeEdited = {id,name,description,image}
}
...
let handleSubmit = (e)=>{
    ...
    formData.append("id",projectToBeEdited.id)
    fetch("/api/projects", {
        method: "PUT",
        body: formData,
        credentials : "include"
